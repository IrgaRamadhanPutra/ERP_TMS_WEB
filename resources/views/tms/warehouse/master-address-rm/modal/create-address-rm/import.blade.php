    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title fs-5" id="importExcelcreateLabel">Import Create Master Address</h4>
...
<script>
    ...
    $(document).on('click', '#btnSave', function(e) {
        ...
        var filePath = inputFile.val(); // Mendapatkan path file
        var allowedExtensions = /(\.xlsx|\.xls|\.csv)$/i; // Ekstensi yang diperbolehkan
        ...
        if (filePath === '') {
            ...
            // Jika file tidak sesuai dengan ekstensi yang diizinkan
            Swal.fire({
                icon: 'error',
                title: 'Error...',
                text: 'Only Excel or CSV files are allowed',
            });
            inputFile.val(''); // Reset input file
        } else {
            // Jika file valid, tampilkan loading
            $('#loading').removeClass('d-none');
            var formData = new FormData($('#importExcelForm')[0]);
            $.ajax({
                type: 'POST',
                url: "{{ route('tms.warehouse.master_address_rm.import_excel_create_master_address') }}",
                data: formData,
                processData: false,
                contentType: false,
