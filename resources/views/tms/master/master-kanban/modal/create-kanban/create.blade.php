                ...
                <div class="col">
                    <div class="modal-body">
                        <form id="form-masterKanban" method="post" action="javascript:void(0)">
                            @csrf
                            @method('POST')
                            @include('tms.master.master-kanban.modal.create-kanban.form')
                        </form>
                    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function clearForm() {
            $('#form-masterKanban').find('input, select').each(function() {
                $(this).val(''); // Clear input and select values
                $(this).removeClass('is-invalid'); // Remove invalid feedback class
            });
        }
                ...
                success: function(response) {
                    ...
                    if (response.ekanban_no) {
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            success: function(response) {
                                if (response.status === 'success' && response.ekanban_no) {
